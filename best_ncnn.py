...
# ----------------------------
# NCNN 모델 폴더 경로
# - .py 파일로 실행: 이 코드 파일과 같은 폴더의 best_ncnn_model 사용
# - Jupyter/Notebook 셀에서 실행: 현재 작업 폴더의 best_ncnn_model 사용
BASE_DIR = (
...
if not os.path.isdir(MODEL_PATH):
    raise FileNotFoundError(
        f"NCNN 모델 폴더가 없습니다: {MODEL_PATH}\n"
        "압축을 푼 best_ncnn_model 폴더를 이 코드와 같은 폴더에 두세요."
    )

print("Loading NCNN YOLO model...")
print("MODEL_PATH:", MODEL_PATH)
yolo_model = YOLO(MODEL_PATH)
print("NCNN YOLO model loaded.")
